...
function autotest.init(name)
    ...
    print("[autotest] running " .. name .. " (" .. #steps .. " steps)")
end
...
function autotest.step()
    ...
    while idx <= #steps do
        ...
        elseif s.call then
            s.call(scene())
        elseif s.log then
            print("[autotest] " .. (type(s.log) == "function" and s.log(scene()) or s.log))
        elseif s.expect then
            local ok, msg = s.expect(scene())
            print(string.format("[autotest] %s %s", ok and "PASS" or "FAIL", msg or ""))
            if not ok then autotest.failed = true end
        elseif s.quit then
            print("[autotest] done" .. (autotest.failed and " WITH FAILURES" or ""))
...
function autotest.afterDraw()
    if pendingShot then
        ...
        print("[autotest] screenshot " .. pendingShot)
        pendingShot = nil
    end
end
